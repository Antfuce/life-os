...
### 2026-02-16 00:20 UTC
- Codex: Resolved merge-artifact drift in backend realtime/docs paths and completed P0 #2/#3/#4 + P1 #5 implementation on a single branch for clean mergeability.
- Added LiveKit bridge endpoints for backend-issued participant tokens and provider-state fan-in to canonical `call.connected|call.ended|call.error|call.terminal_failure` event emission.
- Finalized schema v1 enforcement across emit/replay/checkpoint with legacy alias normalization at ingestion (`timestamp`/`version` accepted, canonicalized to `ts`/`schemaVersion`) and strict reject-on-unknown envelope keys.
- Kept reconnect semantics with resume token validation, sequence-based replay/checkpoint ack persistence, and deterministic orchestration action request + ack/failure events.
- Risk: backend tests still require `node:sqlite`; this environment Node runtime lacks that module, so automated test execution remains blocked.
