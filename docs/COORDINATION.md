...
## 2026-02-20T10:25:50Z — OpenClaw run note (de-monolith phase 1: shared chat modules extraction)
- Changed: Started backend de-monolith extraction without behavior changes by pulling duplicated chat-contract + streaming utility logic out of `server/index.mjs` and `server/index.v2.mjs` into dedicated modules: `server/chat/ui-contract.mjs` (UI event constants, personas, parsers, delta overlap logic) and `server/chat/openclaw-utils.mjs` (gateway token resolution, output extraction, SSE helpers). Wired both server entrypoints to import these shared modules.
- Verification: `node --check server/index.mjs server/index.v2.mjs server/chat/ui-contract.mjs server/chat/openclaw-utils.mjs`, `npm run lint`, `npm run build`, and `node --test server/test/*.test.mjs` all pass.
- Next: Continue Phase 2 by extracting `/v1/chat/stream` + `/v1/chat/turn` route handlers into `server/routes/chat-routes.mjs` and leaving `index.mjs` as composition/wiring only.
- Risks: `index.mjs` remains large; this slice reduces duplication and merge-collision surface but does not yet isolate route-level side effects.
